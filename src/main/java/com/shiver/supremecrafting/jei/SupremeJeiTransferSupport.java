...
public final class SupremeJeiTransferSupport {
    private SupremeJeiTransferSupport() {
    }

    @Nullable
    public static ResourceLocation findRecipeId(mezz.jei.api.gui.IRecipeLayout recipeLayout) {
    ...
    private static Map<Integer, List<ItemStack>> targets(SupremeRecipe recipe) {
    ...
    private static boolean sameInputs(Map<Integer, List<ItemStack>> left, Map<Integer, List<ItemStack>> right) {
    ...
    private static boolean sameCandidates(List<ItemStack> left, List<ItemStack> right) {
        if (left.size() != right.size()) {
            return false;
        }
